  ...
  return (
    <div className="flex items-center gap-2 flex-wrap" dir="rtl">
      ...
      <div className="relative flex items-center">
        <MapPin className="absolute right-8 w-3.5 h-3.5 text-teal-500 pointer-events-none" />
        <select
          value={gov}
          onChange={e => handleGov(e.target.value)}
          className="pr-12 pl-3 py-2 text-sm border border-gray-200 rounded-xl bg-white outline-none focus:ring-2 focus:ring-teal-300 text-gray-700 shadow-sm min-w-[140px]"
        >
      ...
      {/* Unit — always visible */}
      <div className="relative flex items-center">
        <Building2 className="absolute right-8 w-3.5 h-3.5 text-emerald-500 pointer-events-none" />
        <select
          value={unit}
          onChange={e => handleUnit(e.target.value)}
          disabled={!gov}
          className="pr-12 pl-3 py-2 text-sm border border-gray-200 rounded-xl bg-white outline-none focus:ring-2 focus:ring-emerald-300 text-gray-700 shadow-sm min-w-[180px] disabled:opacity-40 disabled:cursor-not-allowed"
        >
